#!/usr/bin/env node
/**
 * Grant `referral.change_marketer` to named users on Aurora (RDS Data API).
 *
 * Usage (from careStream/):
 *   node scripts/grant-change-marketer.js           # dry-run
 *   node scripts/grant-change-marketer.js --apply   # write
 *
 * Requires WB_CLUSTER_ARN + WB_SECRET_ARN (loads careStream/.env if present).
 *
 * Targets (case-insensitive first + last):
 *   Rafi Barides, David Krasner
 */
...
const PERM = 'referral.change_marketer';
const TARGETS = [
  ['rafi', 'barides'],
  ['david', 'krasner'],
];
    ...
    if (APPLY) {
      ...
      await exec(
        `INSERT INTO permissions (id, key, label, category, sort_order, description, created_at, updated_at)
         VALUES (
           'perm_referral_change_marketer',
           :k,
           'Change referral marketer',
           'Referrals',
           14.6,
           'Reassign the marketer on an existing referral from the Referral tab. Deny-by-default.',
