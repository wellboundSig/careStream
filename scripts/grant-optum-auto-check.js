#!/usr/bin/env node
/**
 * Grant `clinical.eligibility_optum_auto` (Optum Auto Check beta) to Rafi only.
 *
 * Usage (from careStream/):
 *   node scripts/grant-optum-auto-check.js           # dry-run
 *   node scripts/grant-optum-auto-check.js --apply   # write
 *
...
const PERM = 'clinical.eligibility_optum_auto';
const TARGETS = [['rafi', 'barides']];
    ...
    if (APPLY) {
      ...
      await exec(
        `INSERT INTO permissions (id, key, label, category, sort_order, description, created_at, updated_at)
         VALUES (
           'perm_clinical_eligibility_optum_auto',
           :k,
           'Optum Auto Check (beta)',
           'Eligibility & Authorization',
           30,
           'Run real-time Optum 270/271 eligibility Auto Check. Deny-by-default until Optum enrollment is validated.',
