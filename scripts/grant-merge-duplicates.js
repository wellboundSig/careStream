#!/usr/bin/env node
/**
 * Grant `referral.merge_duplicates` to named users on Aurora (RDS Data API).
 *
 * Usage (from careStream/):
 *   node scripts/grant-merge-duplicates.js           # dry-run
 *   node scripts/grant-merge-duplicates.js --apply   # write
 *
...
const PERM = 'referral.merge_duplicates';
const TARGETS = [
  ['raquel', 'lipschits'],
  ['rafi', 'barides'],
  ['olesya', 'glukhovskiy'],
    ...
    if (APPLY) {
      ...
      await exec(
        `INSERT INTO permissions (id, key, label, category, sort_order, description, created_at, updated_at)
         VALUES (
           'perm_referral_merge_duplicates',
           :k,
