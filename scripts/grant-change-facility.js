#!/usr/bin/env node
/**
 * Grant `referral.change_facility` to named users on Aurora (RDS Data API).
 *
 * Usage (from careStream/):
 *   node scripts/grant-change-facility.js           # dry-run
 *   node scripts/grant-change-facility.js --apply   # write
 *
 * Requires WB_CLUSTER_ARN + WB_SECRET_ARN (loads careStream/.env if present).
 *
 * Targets (case-insensitive first + last):
 *   Rafi Barides, Mordy Slomovics
 */
...
const PERM = 'referral.change_facility';
const TARGETS = [
  ['rafi', 'barides'],
  ['mordy', 'slomovics'],
];
    ...
    if (APPLY) {
      ...
      await exec(
        `INSERT INTO permissions (id, key, label, category, sort_order, description, created_at, updated_at)
         VALUES (
           'perm_referral_change_facility',
           :k,
           'Change referral facility',
           'Referrals',
           14.7,
           'Change the facility on an existing referral and reconcile marketer, COC nurse, entity, and address. Deny-by-default.',
