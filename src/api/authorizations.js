 ...
 *
 * NOTE: the legacy `services_authorized` column in Aurora still lists ABA
 ...
 * should be removed from the Aurora option list when convenient.
 */
...
export const getAuthorizationsByInsurance = (insuranceId) =>
  aurora.fetchAll(TABLE, { filterByFormula: `FIND("${insuranceId}", ARRAYJOIN({payer_insurance_id}))` });

export const createAuthorization = (fields) => aurora.create(TABLE, normaliseFields(fields));
export const updateAuthorization = (id, fields) => aurora.update(TABLE, id, normaliseFields(fields));
export const deleteAuthorization = (id) => aurora.remove(TABLE, id);
