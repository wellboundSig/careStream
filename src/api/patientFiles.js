...
export const getFilesByReferral = (referralId) =>
  aurora.fetchAll(TABLE, { filterByFormula: `{referral_id} = "${referralId}"`, sort: [{ field: 'created_at', direction: 'desc' }] });
...
export const getFilesForPatients = (patientIds) => {
  ...
  return aurora.fetchAll(TABLE, { filterByFormula: formula });
};
...
export const createFile = (fields) => aurora.create(TABLE, fields);
export const updateFile = (id, fields) => aurora.update(TABLE, id, fields);
export const deleteFile = (id) => aurora.remove(TABLE, id);
