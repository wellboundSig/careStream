...
// A "Referral Source" in our world is a PERSON (e.g. a care manager,
// a discharge planner, a PCP). They sit under a category/type
// (Hospital, CCO, PCP, etc.) and they work for a company / entity
// (e.g. Tri-County Care). All three pieces are captured here so the
// New Referral form can let users pick just the person and auto-display
// the type and company alongside.
// Categories of organization the referrer represents. Communication
// methods (Web / Fax / Allscripts) intentionally do NOT live here — those
// are how a person sent us a referral, not what they are. A PCP or care
// manager can fax something; that doesn't change their type.
const SOURCE_TYPES = [
  'CCO',
  'Hospital',
  'SNF',
...
// Types where it doesn't make sense to require a company / entity
// (patient referred themselves, generic marketing campaign, or unclassified).
const NO_ENTITY_TYPES = new Set(['Self-Referral', 'Campaign', 'Other']);
    ...
    <div style={{ padding: '24px 28px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div>
          ...
          <p style={{ fontSize: 12.5, color: hexToRgba(palette.backgroundDark.hex, 0.5), maxWidth: 540 }}>
            People who refer patients to us — care managers, discharge planners, PCPs, etc. Each row is one <em>person</em> with a category and the company they work for.
          </p>
        </div>
      ...
      <p style={{ fontSize: 12, color: hexToRgba(palette.backgroundDark.hex, 0.4), marginBottom: 16 }}>{enriched.length} {enriched.length === 1 ? 'source' : 'sources'}</p>

      {showCreate && (
        <SourceForm marketers={marketers} onSave={handleCreate} onCancel={() => setShowCreate(false)} />
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: hexToRgba(palette.backgroundDark.hex, 0.025), borderBottom: '1px solid var(--color-border)' }}>
              {['Person', 'Type', 'Company / Entity', 'Marketer', 'Referrals', 'Admissions', 'Conversion', ''].map((h) => (
                <th
                  key={h}
                  style={{ padding: '9px 14px', textAlign: h === '' ? 'right' : 'left', fontSize: 10.5, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: hexToRgba(palette.backgroundDark.hex, 0.4) }}
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {!hydrated ? (
              ...
              <tr>
                <td colSpan={8} style={{ padding: '40px 0', textAlign: 'center', fontSize: 13, color: hexToRgba(palette.backgroundDark.hex, 0.35), fontStyle: 'italic' }}>
                  No sources found.
                </td>
              </tr>
            ) : (
                    ...
                    <tr key={src._id}>
                      <td colSpan={8} style={{ padding: 0 }}>
                        <SourceForm
                          initial={src}
                          marketers={marketers}
                          onSave={(fields) => handleUpdate(src, fields)}
  ...
  return (
    ...
    >
      <td style={{ padding: '11px 14px' }}>
        <p style={{ fontSize: 13.5, fontWeight: 600, color: palette.backgroundDark.hex }}>{src.name || <em style={{ color: hexToRgba(palette.backgroundDark.hex, 0.4) }}>Unnamed</em>}</p>
        {src.id && <p style={{ fontSize: 10.5, color: hexToRgba(palette.backgroundDark.hex, 0.32), marginTop: 1, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>{src.id}</p>}
      </td>
      <td style={{ padding: '11px 14px' }}>
      ...
      <td style={{ padding: '11px 14px', textAlign: 'right' }}>
        {canEdit && hov && (
          <button
            onClick={onEdit}
            style={{ padding: '4px 10px', borderRadius: 5, background: hexToRgba(palette.backgroundDark.hex, 0.05), border: 'none', fontSize: 11, fontWeight: 600, color: hexToRgba(palette.backgroundDark.hex, 0.5), cursor: 'pointer' }}
          >
            Edit
          </button>
        )}
...
// ──────────────────────────────────────────────────────────────────────────────
// SourceForm — used both for inline edit and the create modal-ish panel.
// Captures: person name, type/category, company/entity (conditional), marketer.
// ──────────────────────────────────────────────────────────────────────────────
function SourceForm({ initial, marketers, onSave, onCancel, inline }) {
  ...
  const canSubmit = name.trim() && type && (!entityRequired || sourceEntity.trim()) && !saving;
  ...
  return (
    <div
      style={{
        padding: inline ? '14px 16px' : '18px 20px',
        borderRadius: inline ? 0 : 12,
        background: hexToRgba(palette.backgroundDark.hex, 0.02),
        marginBottom: inline ? 0 : 16,
        border: inline ? `1px solid ${hexToRgba(palette.primaryDeepPlum.hex, 0.18)}` : `1px solid var(--color-border)`,
      }}
    >
      <p style={{ fontSize: 12, fontWeight: 700, color: palette.primaryDeepPlum.hex, marginBottom: 4, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        {initial ? 'Edit Source' : 'New Referral Source'}
      </p>
      <p style={{ fontSize: 11.5, color: hexToRgba(palette.backgroundDark.hex, 0.5), marginBottom: 14, lineHeight: 1.5 }}>
        Add the <strong>person</strong> who refers (e.g. care manager, PCP, discharge planner) and the company they work for. Marketers will pick this person on the Lead Source field of New Referral.
      </p>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 14px', marginBottom: 12 }}>
        <div>
          <label style={{ fontSize: 11.5, fontWeight: 600, color: hexToRgba(palette.backgroundDark.hex, 0.5), display: 'block', marginBottom: 4 }}>
            Person's Name <span style={{ color: palette.primaryMagenta.hex }}>*</span>
          </label>
          <input
            value={name}
            ...
            autoFocus
          />
          <p style={{ fontSize: 10.5, color: hexToRgba(palette.backgroundDark.hex, 0.4), marginTop: 4, lineHeight: 1.45 }}>
            The individual who refers patients. For Self-Referral or Campaign sources, use a short label (e.g. "Self" or the campaign name).
          </p>
        </div>
        <div>
          <label style={{ fontSize: 11.5, fontWeight: 600, color: hexToRgba(palette.backgroundDark.hex, 0.5), display: 'block', marginBottom: 4 }}>
            Type / Category <span style={{ color: palette.primaryMagenta.hex }}>*</span>
          </label>
          ...
          <p style={{ fontSize: 10.5, color: hexToRgba(palette.backgroundDark.hex, 0.4), marginTop: 4, lineHeight: 1.45 }}>
            What kind of organization does this person represent?
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 14px', marginBottom: 12 }}>
        <div>
          <label style={{ fontSize: 11.5, fontWeight: 600, color: hexToRgba(palette.backgroundDark.hex, 0.5), display: 'block', marginBottom: 4 }}>
            Company / Entity {entityRequired && <span style={{ color: palette.primaryMagenta.hex }}>*</span>}
            {entityIsNA && <span style={{ color: hexToRgba(palette.backgroundDark.hex, 0.35), fontWeight: 500 }}> &middot; not applicable</span>}
          </label>
          <input
            value={entityIsNA ? '' : sourceEntity}
            onChange={(e) => setSourceEntity(e.target.value)}
            placeholder={entityIsNA ? '—' : 'e.g. Tri-County Care'}
            disabled={entityIsNA}
            style={{ ...inp, opacity: entityIsNA ? 0.5 : 1, cursor: entityIsNA ? 'not-allowed' : 'text' }}
          />
          <p style={{ fontSize: 10.5, color: hexToRgba(palette.backgroundDark.hex, 0.4), marginTop: 4, lineHeight: 1.45 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 10.5, color: hexToRgba(palette.backgroundDark.hex, 0.4), marginTop: 4, lineHeight: 1.45 }}>
            Optional — the marketer who owns this relationship.
          </p>
        </div>
      </div>

      {/* Live preview of how the source will appear in the New Referral form */}
      {(name.trim() || type) && (
        <div style={{ padding: '10px 12px', borderRadius: 8, background: hexToRgba(palette.accentBlue.hex, 0.06), border: `1px dashed ${hexToRgba(palette.accentBlue.hex, 0.3)}`, marginBottom: 12 }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: hexToRgba(palette.backgroundDark.hex, 0.45), marginBottom: 6 }}>
            Preview
          </p>
          <p style={{ fontSize: 13, fontWeight: 600, color: palette.backgroundDark.hex, marginBottom: 4 }}>
            {name.trim() || <em style={{ fontWeight: 400, color: hexToRgba(palette.backgroundDark.hex, 0.45) }}>Person's name</em>}
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: hexToRgba(palette.backgroundDark.hex, 0.6) }}>
            {type && <TypeBadge type={type} />}
            {!entityIsNA && sourceEntity.trim() && <span>· {sourceEntity.trim()}</span>}
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        <button
          onClick={onCancel}
          style={{ padding: '7px 16px', borderRadius: 7, background: hexToRgba(palette.backgroundDark.hex, 0.05), border: 'none', fontSize: 12.5, fontWeight: 600, color: hexToRgba(palette.backgroundDark.hex, 0.55), cursor: 'pointer' }}
        >
          Cancel
        </button>
        <button
          ...
          style={{
            ...
            fontSize: 12.5,
            fontWeight: 650,
            color: canSubmit ? '#fff' : hexToRgba(palette.backgroundDark.hex, 0.3),
