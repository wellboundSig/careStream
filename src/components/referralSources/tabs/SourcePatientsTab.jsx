...
const COLUMN_DEFS = [
  ...
  { key: 'date',      label: 'Received',  filterable: false },
];

export default function SourcePatientsTab({ referrals, loading }) {
  ...
  const [colFilters, setColFilters] = useState({ division: [], licence: [], stage: [], triage: [], insurance: [] });
  const [sortField, setSortField] = useState('date');
  const [sortDir, setSortDir] = useState('desc');

  function setColFilter(key, val) { setColFilters((p) => ({ ...p, [key]: val })); }
  function clearAll() { setSearch(''); setColFilters({ division: [], licence: [], stage: [], triage: [], insurance: [] }); setStatusFilter('all'); }

  const hasActiveFilters = search.trim() || Object.values(colFilters).some(filterIsActive) || statusFilter !== 'all';
  ...
  }, [referrals, statusFilter, search, colFilters, sortField, sortDir, resolveEntity, triagePresence]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '10px 16px', borderBottom: `1px solid var(--color-border)`, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: hexToRgba(palette.backgroundDark.hex, 0.04), border: '1px solid var(--color-border)', borderRadius: 6, padding: '0 8px', height: 28, minWidth: 140 }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search patient…" style={{ background: 'none', border: 'none', outline: 'none', fontSize: 11.5, color: palette.backgroundDark.hex, width: '100%' }} />
      ...
      <FlipTableShell flip={flip} headerHeight={tabHeaderH} className={lockedGridClass(lockedGrid)} style={{ flex: 1, minHeight: 0 }}>
        {loading ? (
          <p style={{ padding: '32px 22px', fontSize: 13, color: hexToRgba(palette.backgroundDark.hex, 0.4), fontStyle: 'italic', textAlign: 'center' }}>
            Loading patients…
          </p>
        ) : displayed.length === 0 ? (
          <p style={{ padding: '32px 22px', fontSize: 13, color: hexToRgba(palette.backgroundDark.hex, 0.35), fontStyle: 'italic', textAlign: 'center' }}>
            {hasActiveFilters ? 'No patients match the current filters.' : 'No patients have been referred yet.'}
          </p>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: hexToRgba(palette.backgroundDark.hex, 0.025), borderBottom: `1px solid var(--color-border)`, position: 'sticky', top: 0, zIndex: 1 }}>
                {COLUMN_DEFS.map((col) => {
                  const sortMap = { patient: 'name', stage: 'stage', date: 'date' };
                  const field = sortMap[col.key];
                  return (
                    <th
                      key={col.key}
                      className={col.key === 'patient' ? lockColClass(lockedGrid) : undefined}
                      onClick={field ? () => toggleSort(field) : undefined}
                      style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', color: hexToRgba(palette.backgroundDark.hex, 0.4), cursor: field ? 'pointer' : 'default', whiteSpace: 'nowrap' }}
                    >
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                        {col.label}
                        {field && sortField === field && <span style={{ fontSize: 8 }}> {sortDir === 'asc' ? '▲' : '▼'}</span>}
                        {col.filterable && (
                          <span style={{ width: 18, display: 'inline-flex', visibility: showFilters ? 'visible' : 'hidden' }} onClick={(e) => e.stopPropagation()}>
                            {showFilters && (
                              <ColumnFilterButton value={colFilters[col.key]} onChange={(v) => setColFilter(col.key, v)} label={col.label} options={colOptions[col.key] || []} />
                            )}
                          </span>
                        )}
                      </span>
            ...
            <tbody>
              {flip.windowItems.map((ref) => {
                ...
                return (
                  <tr key={ref._id}
                      onClick={() => openPatient({ id: ref.patient_id, _id: ref.patient_id, division: ref.division }, ref)}
                      title="Click to open patient"
                    ...
                    <td className={lockColClass(lockedGrid)} style={{ padding: '9px 12px', fontSize: 13, fontWeight: 550, color: palette.backgroundDark.hex }}>
                      {ref.patientName || ref.patient_id || <em style={{ color: hexToRgba(palette.backgroundDark.hex, 0.4), fontWeight: 400 }}>Unnamed</em>}
                    </td>
      ...
      <div style={{ padding: '8px 16px', borderTop: `1px solid var(--color-border)`, flexShrink: 0, fontSize: 11.5, color: hexToRgba(palette.backgroundDark.hex, 0.4) }}>
        {displayed.length} of {referrals.length} {referrals.length === 1 ? 'patient' : 'patients'}
      </div>
    </div>
  );
}
